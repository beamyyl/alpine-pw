#!/bin/sh
sudo apk add networkmanager networkmanager-wifi networkmanager-tui network-manager-applet networkmanager-dev networkmanager-bluetooth eudev dbus
sudo addgroup -S plugdev 2>/dev/null
sudo addgroup -S netdev 2>/dev/null
sudo adduser "$USER" plugdev
sudo adduser "$USER" netdev

sudo mkdir -p /etc/NetworkManager/conf.d
sudo tee /etc/NetworkManager/NetworkManager.conf > /dev/null << 'EOF'
[main] 
dhcp=internal
plugins=ifupdown,keyfile

[ifupdown]
managed=true 

[device]
wifi.scan-rand-mac-address=yes
wifi.backend=wpa_supplicant
EOF

sudo tee /etc/NetworkManager/conf.d/any-user.conf > /dev/null << 'EOF'
[main]
auth-polkit=false
EOF

sudo rc-service networking stop 2>/dev/null
sudo rc-service wpa_supplicant stop 2>/dev/null
sudo rc-update del networking boot 2>/dev/null
sudo rc-update del wpa_supplicant boot 2>/dev/null

sudo rc-update add udev sysinit
sudo rc-update add dbus boot
sudo rc-update add networkmanager default
sudo rc-service udev start
sudo rc-service dbus start
sudo rc-service networkmanager start
